...
document.addEventListener('DOMContentLoaded', () => {
  ...
  exportBtn.addEventListener('click', () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        showToast('🎉 Excel 补贴模板生成并下载成功！', 'success');
      }, 600);
  ...
  /* 
   * 构造符合 XLSX 规范的数据并应用样式
   * 包含：第1行合并标题行（浅绿背景）、第2行表头（灰底加粗）、精细的实线边框
   */
  function exportToXlsx(records) {
    ...
    const data = [
      ["补贴导入模板", "", "", "", ""],
      ["账户号", "补贴金额", "补贴类型", "到账时间", "备注"]
    ];
    
    records.forEach(r => {
      data.push([
        r.account,
        parseFloat(r.amount), // 补贴金额为数字
        parseInt(r.type),     // 补贴类型为数字
        r.date,
        r.remarks
      ]);
    });
    ...
    // 定制每一行的高度
    ws['!rows'] = [
      { hpt: 30 }, // 第1行 (标题行高度 30pt)
      { hpt: 22 }  // 第2行 (表头行高度 22pt)
    ];
    for (let i = 2; i < data.length; i++) {
      ws['!rows'].push({ hpt: 20 }); // 数据行高度 20pt
    }
    ...
    // 定义各种样式对象
    const titleStyle = {
      font: { name: "宋体", sz: 14, bold: true, color: { rgb: "000000" } },
      fill: { fgColor: { rgb: "E2EFDA" } }, // 绿底
    ...
    const dataStyleAmount = {
      font: { name: "宋体", sz: 11, color: { rgb: "000000" } },
      alignment: { horizontal: "center", vertical: "center" },
      border: borderStyle,
      numFmt: "0.00" // 金额保留两位小数
    };
    
    // 循环应用样式并确保所有单元格都被初始化以获得完整的边框
    ...
    XLSX.utils.book_append_sheet(wb, ws, "补贴导入");
    XLSX.writeFile(wb, "食堂补贴导入模板.xlsx");
  }
